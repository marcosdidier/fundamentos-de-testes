...
DEFAULT_CSV_OUTPUT = ROOT / "results" / "processed" / "execution_results.csv"
DEFAULT_JSONL_OUTPUT = ROOT / "results" / "raw" / "execution_logs.jsonl"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional number of cases to execute before applying the 3 prompt strategies.",
    )
    parser.add_argument(
        "--csv-output",
        type=Path,
...
def main() -> None:
    ...
    client = ClaudeClient()
    records = run_cases(cases, client)
